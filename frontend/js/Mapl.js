...
async function loadMapDisasterData() {
  try {
    const response = await fetch(`${EONET_GEOJSON_URL}?status=open&limit=50`);
...
function updateMapSidebar() {
  ...
  if(currentDisasters.length === 0) {
    container.innerHTML = `
      <div style="padding: 40px; text-align: center; color: #445566;">
        ...
        <div>No active disasters reported</div>
      </div>
    `;
    return;
  }
  ...
  container.innerHTML = `
    <div class="sidebar-header">
      <div class="sidebar-title">NEARBY DISASTERS</div>
    ...
    (position) => {
      ...
      userMarker = L.marker(userCoords, {
        icon: L.divIcon({
          ...
          html: '<div style="width: 20px; height: 20px; background: #00c8ff; border-radius: 50%; border: 3px solid #fff; box-shadow: 0 0 20px #00c8ff;"></div>',
          iconSize: [20, 20],
          iconAnchor: [10, 10]
        })
      }).addTo(map);
      
      userMarker.bindPopup('<strong>Your Location</strong><br>Scanning for nearby disasters...').openPopup();
...
function findNearbyDisasters(userCoords) {
  ...
  if(nearby.length > 0) {
    showToast(`⚠️ Found ${nearby.length} disasters within 1000km`, 'orange');
    ...
    showToast('✅ No disasters detected within 1000km radius', 'green');
